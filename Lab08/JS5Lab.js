 ...
 * ↓ YOUR CODE HERE ↓ */
  ...
  let myGreeting = new Greeting('Ray', 'NJ')
 myGreeting.hello()
...
let yourBook = new Book('Sunrise on the Reaping', 'Suzanne Collins')
...
class Fruit {
  ...
  description() {
    console.log(`A ${this.name} is a ${this.color} and has a ${this.taste} taste.`)
  }
}
  ...
  displayMovies() {
    let movieInfo = ""
     ...
     for(let i = 0; i < this.movies.length; i++) {
      movieInfo += `${this.movies[i].title}, was directed by ${this.movies[i].director}
      `
     }
     return movieInfo
  }
